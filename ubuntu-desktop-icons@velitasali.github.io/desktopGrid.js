...
var DesktopGrid = class {
    ...
    _getEmptyPlaceClosestTo(x, y) {
        ...
        let dockPosition = Prefs.get_dock_position();
        let preferredMinX = 0;
        let preferredMinY = 0;
        ...
        if (dockPosition == 'left')
            ...
            preferredMinY++;
        else if (dockPosition == 'right' && maxColumns > 2) 
            maxColumns--;
        else if (dockPosition == 'bottom' && maxRows > 2)
            maxRows--;

        placeX = DesktopIconsUtil.clamp(placeX, preferredMinX, maxColumns - 1);
        placeY = DesktopIconsUtil.clamp(placeY, preferredMinY, maxRows - 1);
        ...
        for (let column = preferredMinX; column < maxColumns; column++) {
            for (let row = preferredMinY; row < maxRows; row++) {
                let placeholder = this.layout.get_child_at(column, row);
...
var RenamePopup = class {
    ...
    constructor(grid) {
        ...
        this._renameOkButton = new St.Button({
            label: _("OK"),
        ...
        this._renameCancelButton = new St.Button({
            label: _("Cancel"),
            style_class: 'app-view-control button',
            button_mask: St.ButtonMask.ONE | St.ButtonMask.THREE,
            reactive: true,
        ...
        let renameButtonsBox = new St.Widget({
            layout_manager: renameButtonsBoxLayout,
            x_expand: true
        });
        renameButtonsBox.add_child(this._renameCancelButton);
        renameButtonsBox.add_child(this._renameOkButton);

        let renameContentLayout = new Clutter.BoxLayout({
            spacing: 6,
            orientation: Clutter.Orientation.VERTICAL
        });
        let renameContent = new St.Widget({
            style_class: 'rename-popup',
